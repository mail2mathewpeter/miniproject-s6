...
$display_heading1 = array('ID','Caravan Name','Caravan Model','Caravan Type','Caravan Registration No','Amount per day');
$result = mysqli_query($conn, "SELECT caravan_name,caravan_model,caravan_type, caravan_regno, amount FROM caravan") or die("database error:". mysqli_error($conn));
...
foreach($display_heading1 as $heading) {
    ...
    $pdf->Cell(36,8,$heading,1);
    else if($heading=="Caravan Registration No")
    $pdf->Cell(36,8,$heading,1);
    else
...
$phone=$row['amount'];
    ...
    $pdf->Cell(10,10,$k,1);
     ...
     $pdf->Cell(36,10,$logid,1);
   
     $pdf->Cell(36,10,$gender,1);
